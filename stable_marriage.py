...
def wPrefersM1OverM(prefer, w, m, m1):
	...
	for i in range(N):
		# If m1 comes before m in list of w, then w prefers her current engagement, leave
		if (prefer[w][i] == m1):
			return True
	...
	while (freeCount > 0):
		...
		while (m < N):
			if (mFree[m] == False):
				break
			m += 1
		# One by one go to all women according to m's preferences. m - picked free man
		i = 0
